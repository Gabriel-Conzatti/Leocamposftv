...
export const confirmarAula = asyncHandler(async (req, res) => {
    const { aulaId } = req.params;
    ...
    for (const inscricao of aula.inscricoes) {
        ...
        const html = emailAulaConfirmada(inscricao.aluno.nome, aula.titulo, dataFormatada, aula.horario, aula.local, aula.professor?.nome || 'Professor', observacoes || '');
        const enviado = await enviarEmail({
            para: inscricao.aluno.email,
            assunto,
            html,
