...
export const criarPreferencaPagamento = async (dados) => {
    ...
    if (TOKEN && TOKEN.startsWith('APP_USR-')) {
        try {
            ...
            const checkoutRes = await axios.post('https://api.mercadopago.com/checkout/preferences', {
                items: [
                    {
                        title: dados.titulo,
...
export const buscarPagamentosPorReferencia = async (externalReference) => {
    const TOKEN = getToken();
    if (!TOKEN) {
...
export const obterStatusPagamento = async (paymentId) => {
    return { id: paymentId, status: 'pending' };
};
export const verificarWebhookMercadoPago = (signature, body) => {
